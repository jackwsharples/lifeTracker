...
interface WorkItemModalProps {
  classId: string;
  onClose: () => void;
  onSave: (item: Omit<WorkItem, 'id' | 'createdAt' | 'updatedAt'>) => void;
}

const WorkItemModal: React.FC<WorkItemModalProps> = ({ classId, onClose, onSave }) => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50 overflow-y-auto ios-momentum">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          <h3 className="text-lg font-semibold text-gray-900">Add Work Item</h3>
          <button
            onClick={onClose}
